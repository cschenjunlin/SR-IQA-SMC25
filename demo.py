...
session = str(1)
pseudo_weight = 0.01
ckpt = f'./checkpoints/LIQE-SR_{pseudo_weight}.pt'
...
def liqe_test_csv():
    for idx, image_row in tqdm(df.iterrows(), total=df.shape[0], desc=f"Computing 2AFC scores of liqe_quality"):
        group = image_row['group'].upper()
        pair = image_row['sr_pair_names']
        dir_path = os.path.join('./RealSR-1K', f'{group}_images', pair)
        ...
        new_column = f'LIQE-SR_{pseudo_weight}'
